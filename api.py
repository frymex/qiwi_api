...
def _gen_headers():
    headers_sys = {
        ...
        'accept-language': 'ru',
        'authorization': f'TokenHeadV2 {z_py.web_qw}',
...
def _gen_reg_platform():
    cookies = {
        ...
        'auth_ukafokfuabbuzdckyiwlunsh': 'MDI1fF98X3xcMQhGAm0EaV4Lb30Pf38zAkVkTgo2aHwgDHwUUXERVzw7XwJJZHsMcQBXRltcdEtGDydDfkYCLkdQZVB7Y1BFWDBAEwc6VHdCVjp8WDR+YAJJKkVCYm9kfV57RwN0Qg==',
    }
...
class Qiwi:
    ...
    def __init__(self, phone_number: [str, int, None], password: [str, None], qiwi_v2=None):
        z_py.password, z_py.phone = password, phone_number
        self.cookies = None
        self.widgetAliasCode = None

        self.captcha = TwoCaptcha(z_py.ru_captcha_key, server='rucaptcha.com')
        ...
        if not qiwi_v2:
            ...
            z_py.web_qw = qiwi_v2.get('web-qw')
            x = RequestsCookieJar()
    ...
    async def save_account(self):
        with requests.Session() as c:
            ...
            cookies = {
                ...
                'auth_ukafokfuabbuzdckyiwlunsh': 'MDAyfF98X3wAV2IBSS17fmwlRlVxVVl1CXVkAFIvI38VYVxmL1l+C1AEAFd+e09sTEhHV39aZ3J+S20QC14CLGQRbioCXmAKWlN8A04seGdyJRNQdR4LewxxLAJPenNnS2xYZHdedQ==',
            }
            ...
            data = {
                ...
                'username': f'+{str(z_py.phone)}',
                'password': f'{z_py.password}',
            }

            response = c.post('https://qiwi.com/oauth/token', cookies=cookies, headers=headers, data=data)
            json_: dict = response.json()
            if json_.get('error', None):
                err_text = json_.get('user_message', None)
                exit(f'[!] Qiwi error: {err_text}')

            cookies = response.cookies
            self.cookies = cookies

            token1 = json_.get('access_token', None)
            token2 = json_.get('refresh_token', None)
            ...
            z_py.web_qw = base64_message

    def get_balance(self) -> Union[Accounts, int]:

        response_sys = requests.get(Qiwi.accounts_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies)

        if response_sys.ok:

            return Accounts(response_sys.json())

        else:
            return response_sys.status_code

    def get_identifications(self) -> Identifications:

        response_sys = requests.get(Qiwi.identifications_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies)

        return Identifications(response_sys.json())

    def get_available_access_methods(self):

        response_sys = requests.get(Qiwi.access_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies)

        return Access(response_sys.json())
    ...
    def create_p2p_keys(self, pair_name: str) -> NewPairP2p:
        ...
        response_sys = requests.post(Qiwi.p2p_create_keys_obj,
                                     headers=_gen_headers(), cookies=self.cookies, json=json_data)

        return NewPairP2p(response_sys.json())

    def get_my_p2p_keys(self) -> MyP2P_Keys:

        response_sys = requests.get(Qiwi.p2p_key_obj,
                                    headers=_gen_headers(), cookies=self.cookies)

        return MyP2P_Keys(response_sys.json())

    def block_p2p_key(self, public_key: str):

        json_data = {
            'publicKey': f'{public_key}',
        }

        response_sys = requests.post(Qiwi.p2p_block_key_obj,
                                     headers=_gen_headers(), cookies=self.cookies, json=json_data)
        if response_sys.status_code == 500:
            return False
        else:
            return True
    ...
    def _gen_public_key_p2p(self) -> Union[bool, Any]:
        ...
        response_sys = requests.post('https://edge.qiwi.com/widgets-api/api/p2p/protected/generate-public-key',
                                     headers=_gen_headers(), cookies=self.cookies)

        if response_sys.status_code == 500:
            return False
        else:
            self.widgetAliasCode = response_sys.json().get('widgetAliasCode')
            return response_sys.json()
    ...
    def create_invoice(self, amount: int, comment: str = 'Qiwi Client By @cazqev') -> Union[bool, Any]:
        ...
        response_sys = requests.post(Qiwi.p2p_create_invoice_obj,
                                     headers=_gen_headers(), cookies=self.cookies, json=json_data)

        if response_sys.status_code == 500:
            return False
        else:
            return response_sys.json().get('invoice_uid')
    ...
    def get_history(self, rows: int = 10) -> dict:
        response_sys = requests.get(Qiwi.history_obj.format(z_py.phone, rows),
                                    headers=_gen_headers(), cookies=self.cookies)
        if response_sys.ok:
            return response_sys.json()
        else:
            exit(response_sys.json())
    ...
    def get_nickname(self) -> Nickname:
        ...
        response_sys = requests.get(Qiwi.nickname_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies)

        return Nickname(response_sys.json())
    ...
    def gen_nickname(self) -> Nickname:
        ...
        response_sys = requests.put(Qiwi.nickname_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies, json=json_data)
    ...
    def get_email(self):
        response_sys = requests.get(Qiwi.email_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies)
    ...
    def add_email(self, email: str, change_password_via_email: bool = True, get_promos_to_email: bool = False):
        ...
        response_sys = requests.put(Qiwi.email_obj.format(z_py.phone),
                                    headers=_gen_headers(), cookies=self.cookies, json=ix_json)
    ...
    def confirm_email(self, code: [str, int]):
        ...
        ix_json = {'code': code}
        response_sys = requests.post(Qiwi.confirm_email_obj.format(z_py.phone),
                                     headers=_gen_headers(), cookies=self.cookies, json=ix_json)

        return response_sys.json().get('status')
    ...
    def create_account(self, phone_number: [str, int], logs: bool = False) -> Union[bool, Any]:
        ...
        if response1.ok:
            return response1.json().get('confirmation_id')
        else:
            return False
    ...
    def submit_qiwi_reg(self, code: [str, int], confirmation_id: [str, int], phone: [str, int],
        ...
        if response1.ok:
            ...
            cookies = {
                ...
                'ref': 'newsite_index_1',
                'auth_ukafokfuabbuzdckyiwlunsh': 'MDA1fF98X3wGNVJ4MVgPYGNYDwcAG0AREFRVQFALUE4KKHc1FnQWew4YcGhoB2Z/CXRaZVJ+aEpqPVhxKhANFEkXFEdlWgBjA2MadmBUWH58VFwDUgJDREMAGh0YX1dWV3pxZkR3SA==',
            }
            ...
            if response.ok:
                z_py.phone = str(phone)
                z_py.password = password
    ...
    def update_verification(self, birth_date: str, country: str = 'RU',
                            first_name: str = '', last_name: str = '',
                            middle_name: str = '', passport: str = '',
                            ):
        pass
